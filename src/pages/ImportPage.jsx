...
const ImportPage = () => {
  ...
  const parseCards = (text) => {
    ...
    const parsedCards = lines.map((line, index) => {
      ...
      // Extract image URLs from front and back
      const urlRegex = /(https?:\/\/[^\s]+)/
      
      // Process front
      const frontUrlMatch = front.match(urlRegex)
      if (frontUrlMatch) {
        card.frontImageUrl = frontUrlMatch[1]
        card.front = front.replace(frontUrlMatch[1], '').trim()
      } else {
        card.front = front.trim()
      }
      
      // Process back
      const backUrlMatch = back.match(urlRegex)
      if (backUrlMatch) {
        card.imageUrl = backUrlMatch[1] // Keep this for backward compatibility
        card.backImageUrl = backUrlMatch[1]
        card.back = back.replace(backUrlMatch[1], '').trim()
      } else {
        card.back = back.trim()
      }
    ...
    <div className={styles.importDecks}>
      ...
      <div className={`${styles.importForm} glass rounded-lg`}>
        ...
        <div className={styles.formSection}>
          ...
          <textarea
            id="importText"
            placeholder={`Format: front\tback or front,back\nOr: "front text" "back text"\nImages can be included in text as URLs\n\nExample:\n${sampleData}`}
            value={importText}
            onChange={(e) => setImportText(e.target.value)}
            rows={8}
          />
          <div className={styles.textareaHint}>
            Supports quoted text, URLs in text, and auto-detects separators (tab/comma/space)
          </div>
        </div>
